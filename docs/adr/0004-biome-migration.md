...
* **Migrate to Biome with allowlist** — Single tool for lint + format; `files.includes: ["**/*.ts", "**/*.tsx", "**/*.mjs"]` ensures only TypeScript/JS are processed. Markdown and other files are never touched.
* **Keep oxlint/oxfmt, ignore all markdown** — Same as first option; already applied as a quick fix in docs PR.

## Decision Outcome

Chosen option: **Migrate to Biome with allowlist**, because it prevents surprises: only explicitly included file types (`.ts`, `.tsx`, `.mjs`) are linted and formatted. Documentation changes cannot trigger format failures.
