...
- **This directory:** `npm run test:integration`
- **Optional live Gmail:** `GMAIL_TEST_EMAIL=... GMAIL_APP_PASSWORD=... npm run test:integration` (requires 2FA + app password; skips when unset)
- **Optional keyring test:** `KEYRING_TEST=1 npm run test:integration` (verifies system keychain; skips when unset)
